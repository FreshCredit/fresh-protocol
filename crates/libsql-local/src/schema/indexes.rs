...
pub async fn initialize_plaid_indexes(conn: &Connection) -> Result<()> {
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_employment_user_id ON employment(user_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_enrich_transaction_id ON enrich(transaction_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_income_user_id ON income(user_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_income_verification_user_id ON income_verification(user_id)").await?;
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_investments_holdings_account_id ON investments_holdings(account_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_investments_securities_ticker ON investments_securities(ticker_symbol)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_investments_transactions_account_id ON investments_transactions(account_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_layer_account_id ON layer(account_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_liabilities_account_id ON liabilities(account_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_monitor_item_id ON monitor(item_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_recurring_transactions_account_id ON recurring_transactions(account_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_signal_evaluations_account_id ON signal_evaluations(account_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_statements_account_id ON statements(account_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_transactions_sync_item_id ON transactions_sync(item_id)").await?;
    Ok(())
}
...
pub async fn initialize_payment_indexes(conn: &Connection) -> Result<()> {
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_stripe_plaid_payments_user_id ON stripe_plaid_payments(user_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_virtual_accounts_customer_id ON virtual_accounts(customer_id)").await?;
    Ok(())
}
...
pub async fn initialize_business_indexes(conn: &Connection) -> Result<()> {
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_offers_user_id ON offers(user_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_offers_provider_id ON offers(provider_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_disputes_user_id ON disputes(user_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_verification_requests_user_id ON verification_requests(user_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_phone_verification_codes_phone ON phone_verification_codes(phone_number)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_phone_verification_codes_expires ON phone_verification_codes(expires_at)").await?;
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_identities_account_id ON identities(account_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_identities_user_id ON identities(user_id)").await?;
    Ok(())
}
...
pub async fn initialize_ticketing_indexes(conn: &Connection) -> Result<()> {
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_tickets_type ON tickets(ticket_type)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_tickets_created_at ON tickets(created_at)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_ticket_comments_ticket_id ON ticket_comments(ticket_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_ticket_comments_author_id ON ticket_comments(author_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_ticket_assignments_ticket_id ON ticket_assignments(ticket_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_ticket_assignments_assignee_id ON ticket_assignments(assignee_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_ticket_sla_events_ticket_id ON ticket_sla_events(ticket_id)").await?;
    Ok(())
}
...
pub async fn initialize_compliance_indexes(conn: &Connection) -> Result<()> {
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_compliance_rules_framework ON compliance_rules(framework)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_compliance_rules_severity ON compliance_rules(severity)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_compliance_rules_is_active ON compliance_rules(is_active)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_compliance_findings_scan_id ON compliance_findings(scan_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_compliance_findings_rule_id ON compliance_findings(rule_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_compliance_findings_severity ON compliance_findings(severity)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_compliance_findings_status ON compliance_findings(status)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_compliance_evidence_finding_id ON compliance_evidence(finding_id)").await?;
    Ok(())
}
...
pub async fn initialize_platform_indexes(conn: &Connection) -> Result<()> {
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_referrals_referrer_user_id ON referrals(referrer_user_id)").await?;
    try_create_index(conn, "CREATE UNIQUE INDEX IF NOT EXISTS idx_referrals_code ON referrals(referral_code)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_referrals_status ON referrals(status)").await?;
    try_create_index(conn, "CREATE UNIQUE INDEX IF NOT EXISTS idx_platform_metrics_date ON platform_metrics(metric_date)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_sales_pipeline_user_id ON sales_pipeline(user_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_sales_pipeline_stage ON sales_pipeline(stage)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_sales_pipeline_hubspot_deal_id ON sales_pipeline(hubspot_deal_id)").await?;
    Ok(())
}
...
pub async fn initialize_linkedin_indexes(conn: &Connection) -> Result<()> {
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_linkedin_skills_profile ON linkedin_skills(linkedin_profile_id)").await?;
    Ok(())
}
...
pub async fn initialize_healthkit_indexes(conn: &Connection) -> Result<()> {
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_healthkit_records_date ON healthkit_records(start_date)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_healthkit_workouts_profile ON healthkit_workouts(healthkit_profile_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_healthkit_activity_profile ON healthkit_activity_summaries(healthkit_profile_id)").await?;
    Ok(())
}
...
pub async fn initialize_correlation_indexes(conn: &Connection) -> Result<()> {
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_correlation_metrics_user ON correlation_metrics(user_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_correlation_metrics_type ON correlation_metrics(metric_type)").await?;
    Ok(())
}
...
pub async fn initialize_apple_music_indexes(conn: &Connection) -> Result<()> {
    ...
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_apple_music_playlists_user ON apple_music_playlists(user_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_apple_music_recently_played_user ON apple_music_recently_played(user_id)").await?;
    try_create_index(conn, "CREATE INDEX IF NOT EXISTS idx_apple_music_genre_stats_user ON apple_music_genre_stats(user_id)").await?;
    Ok(())
}
