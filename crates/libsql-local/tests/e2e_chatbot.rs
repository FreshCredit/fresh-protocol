...
async fn test_message_persistence() -> Result<()> {
    ...
    let conversation_id = client
        .create_conversation(&test_data.user_id, Some("Message Test"), None)
        .await?;
    println!("✅ Conversation created");

    // Add user message
    let user_msg_id = client
...
async fn test_bucketed_data_access() -> Result<()> {
    ...
    let mut total_balance = client
        ...
        .await?;
    assert!(
        has_rows(&mut total_balance).await,
...
async fn test_conversation_history() -> Result<()> {
    ...
    assert_eq!(
        conv1_messages.len(),
        3,
        "Conversation 1 should have 3 messages"
    );
    ...
    assert_eq!(
        conv2_messages.len(),
        2,
        "Conversation 2 should have 2 messages"
    );
...
async fn test_message_limit() -> Result<()> {
    ...
    // Create a conversation
    let conversation_id = client
        .create_conversation(&test_data.user_id, Some("Limit Test"), None)
        .await?;
    ...
    let limited_messages = client
        .get_conversation_messages(&conversation_id, Some(5))
        .await?;
    assert_eq!(
        limited_messages.len(),
        5,
        "Should have 5 messages with limit"
    );
    println!(
        "✅ Retrieved {} messages with limit",
        limited_messages.len()
    );
