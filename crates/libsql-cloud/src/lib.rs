...
//!
//! // HARDCODED_SCHEMA: 87 tables in unified_schema.sql (86 Rust + 1 browser-specific blockchain_proofs) (verified 2026-01-02)
//! Uses the unified 87-table schema from migrations/unified_schema.sql.
...
/// // HARDCODED_SCHEMA: 87 tables (verified 2026-01-02)
...
impl CloudClient {
    ...
    pub async fn initialize_schema(&self) -> Result<()> {
        info!("Initializing cloud database with unified schema (87 tables)");
    ...
    pub async fn get_user_profile_by_azure_id(
            ...
            .map_err(|e| freshcredit_types::FreshCreditError::DatabaseError(e.to_string()))?
        {
            Ok(Some(freshcredit_libsql_local::UserProfile {
                ...
                tenant_id: row.get(22).unwrap_or_default(),
                object_id: row.get(23).unwrap_or_default(),
                verified_id_credential_id: row.get(24).ok(),
                verified_id_status: row.get(25).unwrap_or_else(|_| "pending".to_string()),
                verified_id_issued_at: row.get(26).ok(),
